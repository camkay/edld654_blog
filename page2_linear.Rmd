...
The linear model provided an introduction to machine learning and a general estimate of the RMSE model fit based on all of our predictor variables. Using the linear model, our predictors were able to account for `r round(holdout_rsq$.estimate*100, 2)`% of variance in our score with an RMSE of `r round(holdout_rmse$.estimate, 2)`. Overall, the linear model was extremely quick to run computationally as the algorithm utilized was `lm` and there were no hyperparameter tuning involved at this most basic level. 

For our subsequent model,s we will explore non-linear models that can be tuned for an optimal bias-variance balance. 
