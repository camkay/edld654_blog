...
Overall, the linear model was extremely quick to run computationally (only took `r round(lin_time, 2)` to run through our 10-fold CV) as the algorithm (`lm`) makes a lot of assumptions about the data (i.e., linear model has a high bias) and there were no hyperparameter tuning involved in this basic model (we will briefly touch upon penalized regression which allwos for tuning in the seciton below). Given that the linear model assumes high assumption of our data, the linear model will have relatively low variance when applied to new unseen data. However, given the high-bias and low-variance of the linear model, we will generally expect the model to produce consistent prediction that may not be the most accurate. 

To improve our RMSE, we will explore application of non-linear models - [Random Forest](page3_rf.html) and [Boosted Tree](page4_xgboost.html) - to our data that can be tuned for an optimal bias-variance balance. 
