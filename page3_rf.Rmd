...
Whew! It only took `r unname(time1$toc - time1$tic)` seconds to run.

Now we can check how it fit using the `collect_metrics()` function. 

```{r}
collect_metrics(fit_rf) # 86.2
```

We can see that it had an RMSE of `r spround(collect_metrics(fit_rf)[1, "mean"][[1]])` and a $R^2$ of `r spround(collect_metrics(fit_rf)[2, "mean"][[1]])`. In other words, our predictors explained `r spround(collect_metrics(fit_rf)[2, "mean"][[1]]*100)`% of the variance in the scores.
...
As with the [linear model](page2_linear.html), we can also tune our model. With random forest models, there are three hyperparameters. The first is the number of `trees`, which we set to `500` in the previous model. However, with random forest models, people just have to ensure that there are enough trees (there is not the issue of too many trees). As a result, people commonly only tune the other two hyperparameters. 
...
This time, it took `r unname(time2$toc - time2$tic)` seconds to run, but it is not surprising it took longer, because we fit five times the models (because we provided 5 hyperparameter values to test).
...
We can see that the best fitting model samples `r spround(show_best(fit_rf_tune, metric = "rmse", n = 10)[1, "min_n"][[1]])` predictors for each tree and has an RMSE of `r spround(show_best(fit_rf_tune, metric = "rmse", n = 10)[1, "mean"][[1]])`, an improvement over our previous RMSE of `r spround(collect_metrics(fit_rf)[1, "mean"][[1]])`.
...
The new model took `r unname(time3$toc - time3$tic)` seconds to fit. Again, it makes sense that it is longer, because we are now testing `10` hyperparameters instead of `5`.
...
It looks like the best `min_n` (of the values we tested) is `r spround(show_best(fit_rf_tune_2, metric = "rmse", n = 10)[1, "min_n"][[1]])` which resulted in an average model RMSE of `r spround(show_best(fit_rf_tune_2, metric = "rmse", n = 10)[1, "mean"][[1]])`. This is marginally better than our previous best RMSE of `r spround(show_best(fit_rf_tune, metric = "rmse", n = 10)[1, "mean"][[1]])`.

# Finalizing the model

Now that if we have promising hyper parameters, we can fit our model on the left out testing data. To do so, we will first want to finalize the workflow (using `finalize_workflow()`) to use the most performant hyperparameters.
...
Unsurprisingly, it was much faster to fit the model, coming in at `r unname(time3$toc - time3$tic)` seconds, because we only were not testing multiple hyperparameters.

```{r}
collect_metrics(fit_rf_final) # 87.2
```

The RMSE for the assessment set was <!--`r spround(collect_metrics(fit_rf_final)[1, ".estimate"][[1]])`).-->

# Final thoughts

Although the final model on the assessment set had a greater RMSE  (i.e., <!--`r spround(collect_metrics(fit_rf_final)[1, ".estimate"][[1]])`-->) than the last model fit on the analysis set (i.e., `r spround(show_best(fit_rf_tune_2, metric = "rmse", n = 10)[1, "mean"][[1]])`), but it is not drastically so. We can, therefore, likely conclude that the model has relatively low variance.

In terms of final thoughts about tuning the model, even when only using `500` `trees`, testing only `15` different values for the minimum node size (i.e., `min_n`), and using only `10`% of the data, the tuning model took over `35` minutes to run on a computer with `r benchmarkme::get_cpu()$no_of_cores`  `r str_extract(benchmarkme::get_cpu()$model_name, "\\d\\.\\d{2}GHz")` cores (hyper-threaded) with `r benchmarkme::get_ram()` of RAM. Moreover, the model RMSE also only improved by <!--`r spround(collect_metrics(fit_rf)[1, "mean"][[1]] - show_best(fit_rf_tune_2, metric = "rmse", n = 10)[1, "min_n"][[1]])`-->. At least for this data, the additional time required to tune the model did not seem appropriate for the minimal increase in RMSE.  
