...
Only 5% of the training data was used was used so the model tuning wasn't too computationally demanding. 
...
Note the cross-validated boosted tree model with default parameters took about 50s. Will proceed with caution.
...
Even with the default parameters, the mean RMSE across the test sets is relatively good (`r mt_rmse$test_rmse_mean`). Let's see how well the model will do with some tuning!
...
```{r}
grid_tree = expand.grid(num_tree = seq(50, 500, length.out = 5))
...
```{r}
grid_learn <- expand.grid(learn_rate = seq(.001, .1, length.out = 5))
